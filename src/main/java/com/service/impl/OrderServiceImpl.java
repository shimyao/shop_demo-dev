...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse createOrder(Integer userId,Integer shippingId) {
        ...
        Shipping result=shippingMapper.selectByPrimaryKey(shippingId);
        if(result==null){
    ...
    public ServerResponse send_goods(Integer userId, Long orderNo) {
       //参数非空校验
        ...
        if (order.getStatus()!=Const.OrderStatusEnum.ORDER_PAYED.getCode()){
            return ServerResponse.serverResponseByError("该订单发货失败");
        }
        //发货，更改状态status，发货时间send_time
